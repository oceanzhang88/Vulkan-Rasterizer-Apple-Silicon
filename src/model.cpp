...
namespace std {
    template<>
    struct hash<Ocean::Model::Vertex> {
        size_t operator()(Ocean::Model::Vertex const &vertex) const {
            size_t seed = 0;
            Ocean::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
            return seed;
        }
    };
}  // namespace std
